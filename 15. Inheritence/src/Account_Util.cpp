...
{
    std::cout << "\n************************ \n";
    std::cout << "\nDisplaying the Checking Accounts: \n";
    for (int i=0; i < accounts.size(); i++)
    {
        std::cout << std::endl << i + 1<<" : " << accounts.at(i) << std::endl;
    }
}

void deposit(std::vector<Checking_Account> &accounts, double amount)
{
    std::cout << "\nDepositing to All Checking Accounts...\n";
    for (auto &acc:accounts)
    {
        if (acc.deposit(amount))
            std::cout << "Deposited " << amount << " to " << acc << std::endl;
...
{
    std::cout << "\nWithdrawing from ALL checking accounts...\n";
    for (auto &acc:accounts)
    {
        if (acc.withdraw(amount))
            std::cout << "Successful.\n";
...
{
    std::cout << "\nDepositing to All trust Accounts...\n";
    for (auto &acc:accounts)
    {
        if (acc.deposit(amount))
            std::cout << "Deposited " << amount << " to " << acc << std::endl;
...
{
    std::cout << "\nWithdrawing from ALL trust accounts...\n";
    for (auto &acc:accounts)
    {
        if (acc.withdraw(amount))
            std::cout << "Successful.\n";
